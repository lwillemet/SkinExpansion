...
global run
global image_times, data_times, forcesvec, posvec, anglevec, targetvec, pwmvec, DIR_trial
...
repeat = 10
friction_cond = np.repeat(range(0,5),repeat)
...
try:
    ...
    for ii in range(len(Plan)):
        ...
        if save==True:
            ...
            with open(filename_answer, "a") as fa:
                writer = csv.writer(fa)
                if ii==0:
                    writer.writerow(["friction","correct","answer"])
                #for i in range(len(Plan)):
                writer.writerow([Plan[ii,0]+Plan[ii,1],most_slippery[ii],answer[ii]])
            fa.close()
